...
function project.build()
  ...
  vim.cmd(string.format(":lua require('project').async_task(\"%s\")",
          string.format("source project.sh && messanger_build_%s", items[2])))
end

function project.run()
  if items[2] == "debug" then
    vim.cmd(string.format(":call HTerminal(0.4, 200, \"source project.sh && messanger_run_debug\")"))
  else
    vim.cmd(string.format(":call HTerminal(0.4, 200, \"source project.sh && messanger_run_release\")"))
  end
end
...
function project.clean()
  ...
  vim.cmd(string.format(":silent; lua require('project').async_task(\"%s\")",
    string.format("source project.sh && messanger_clean")))
end
